...
def hub_tests():
    ...
    # test case 4: a frame created by myself
    mytest = mk_pkt("20:00:00:00:00:01", "ff:ff:ff:ff:ff:ff", "192.168.1.100",'172.16.42.2')
    s.expect(PacketInputEvent("eth1", mytest, display=Ethernet),"An Ethernet frame with a broadcast destination address should arrive on eth1")
    s.expect(PacketOutputEvent("eth0", mytest, "eth2", mytest, display=Ethernet),"The Ethernet frame with a broadcast destination address should be forwarded out ports eth0 and eth2")
    return s
